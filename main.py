...
while True:
    ...
    if mode == "t":
        ...
        for w in zoekterm:
            translated = keys_trans.translate(w, src='nl', dest='en')
            keywords += f'"{translated.text}", '

        msg = f"Write a {int(n_words)}+ word text. Make it {style[0]} and {style[1]}, don't make it too commercial " \
              f"or advertising. It should contain the keyword(s) {keywords} the keywords should be included VERBATIM. " \
              f"Divide the text in {int(n_pars)} paragraphs."
